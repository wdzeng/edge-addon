...
async function getAccessToken(clientId, clientSecret, accessTokenUrl) {
    ...
    const response = await axios_1.default.post(accessTokenUrl, formData);
    const accessToken = response.data.access_token;
    core.info('Access token got.');
    core.debug('Access token: ' + accessToken);
    return accessToken;
}
async function uploadPackage(productId, zipPath, token) {
    ...
    const zipStream = fs_1.default.createReadStream(zipPath);
    let response = await axios_1.default.post(url, zipStream, { headers: { 'Content-Type': 'application/zip', Authorization: `Bearer ${token}` } });
...
async function sendSubmissionRequest(productId, token) {
    ...
    let response = await axios_1.default.post(url, {}, { headers: { Authorization: `Bearer ${token}` } });
    const operationId = response.headers.location;
    ...
    switch (errorCode) {
        case 'SubmissionValidationError':
            ...
            if (errors) {
                errors.forEach((e) => core.setFailed(JSON.stringify(e)));
            }
            else {
...
async function run(productId, zipPath, clientId, clientSecret, accessUrl, uploadOnly) {
...
"use strict";
module.exports = JSON.parse('{"name":"edge-addon","description":"Publish to Microsoft Edge Add-ons.","version":"v1.1.0","author":"Hyperbola","private":true,"repository":{"type":"git","url":"git+https://github.com/wdzeng/edge-addon"},"scripts":{"build":"tsc && ncc build ./dist/index.js && cp action.yml dist","clean":"rm -rf dist"},"dependencies":{"@actions/core":"^1.10.0","axios":"^1.4.0"},"devDependencies":{"@types/node":"^16.18.31","@vercel/ncc":"^0.36.1","typescript":"^5.0.4"},"license":"MIT"}');
